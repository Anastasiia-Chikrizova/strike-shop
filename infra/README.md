...
└── live/
    ├── app/            root module: network + app-instance
    └── eks/            root module: network + eks-demo
```

Two root modules, not one, and that split is the whole point: `live/app` stays
up for months at ~$6/month, `live/eks` exists for hours at a time. One combined
state would make it impossible to destroy the expensive half on its own.
...
The `live/eks` numbers are why it is a separate root module: one combined state
